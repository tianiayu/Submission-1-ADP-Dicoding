...
def load_data():
    hour_df = pd.read_csv("D:\Bootcamp\Laskar AI\Submission\submission\data\hour.csv")
    day_df = pd.read_csv("D:\Bootcamp\Laskar AI\Submission\submission\data\day.csv")
...
with col1:
    st.metric("Total Peminjaman Sepeda", int(filtered_day_df["cnt"].sum()))
with col2:
...
if not time_category_totals.empty:
    ...
    plt.pie(time_category_totals["cnt"], labels=time_category_totals["time_category"], 
